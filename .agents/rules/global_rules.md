...
- **Medical Advertising Laws:** All output must strictly adhere to `/00_Compliance/EOO_Guidelines.md`. Avoid making "guaranteed results" claims. Always use qualifying language like "potential results," "individual outcomes may vary," and "subject to clinical assessment."
...
- **Context Verification:** Before generating any output, the Manager must verify the current working directory. 
    - If in `/01_Surgical_Division/`, strictly follow `surgical_brand_identity.md`.
    - If in `/02_Cosmetic_Division/`, strictly follow `cosmetic_brand_identity.md`.
